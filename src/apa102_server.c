...
static int spi_fd = -1;
static u8 spi_data_tx[((1 << 16) / 3) * 4 + 9];
static u32 spi_speed_hz = SPI_DEFAULT_SPEED_HZ;


void apa102_put_pixels(int fd, u8 spi_data_tx[], u16 count, pixel* pixels) {
  int i;
  pixel* p;
  u8* d;
  u8 flag;
...
void handler(u8 address, u16 count, pixel* pixels) {
  ...
  fflush(stderr);
  apa102_put_pixels(spi_fd, spi_data_tx, count, pixels);
}
...
int main(int argc, char** argv) {
  ...
  apa102_put_pixels(spi_fd, spi_data_tx, 1, &diagnostic_pixel);
}
